...
====================
*/
qboolean CL_IsThirdPersonE( void )
{
...
#ifdef __vita__
static int tri_mode = 0;
static int tri_numverts = 0;
static float *tri_vertp = NULL;
static float *tri_colorp = NULL;
static float *tri_texp = NULL;
#endif
...
#ifdef __vita__
	if( mode == GL_POLYGON || mode == GL_QUADS )
		mode = GL_TRIANGLE_FAN;
	else if ( mode == GL_QUAD_STRIP )
		mode = GL_TRIANGLE_STRIP;
	tri_mode = mode;
	tri_vertp = gl_vgl_verts;
	tri_colorp = gl_vgl_colors;
	tri_texp = gl_vgl_texcoords;
	tri_numverts = 0;
#else
	pglBegin( mode );
#endif
...
{
#ifdef __vita__
	*(tri_vertp++) = v[0]; *(tri_vertp++) = v[1]; *(tri_vertp++) = v[2];
	tri_numverts++;
#else
	pglVertex3fv( v );
#endif
...
{
#ifdef __vita__
	*(tri_vertp++) = x; *(tri_vertp++) = y; *(tri_vertp++) = z;
	tri_numverts++;
#else
	pglVertex3f( x, y, z );
#endif
