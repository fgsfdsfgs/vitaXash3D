	...
	float width, float scale, float freq, float speed, int segments, int flags, float *color )
{
...
#ifdef __vita__
			*(colp++) = curSeg.color[0]; *(colp++) = curSeg.color[1];
			*(colp++) = curSeg.color[2]; *(colp++) = curSeg.alpha;
			*(uvp++) = 0.f; *(uvp++) = curSeg.texcoord;
			*(vertp++) = vPoint1[0]; *(vertp++) = vPoint1[1]; *(vertp++) = vPoint1[2];
			numverts++;

			*(colp++) = curSeg.color[0]; *(colp++) = curSeg.color[1];
			*(colp++) = curSeg.color[2]; *(colp++) = curSeg.alpha;
			*(uvp++) = 1.f; *(uvp++) = curSeg.texcoord;
			*(vertp++) = vPoint2[0]; *(vertp++) = vPoint2[1]; *(vertp++) = vPoint2[2];
			numverts++;
#else
	...
	Vita_ColorPointer( 4, GL_FLOAT, 0, numverts, gl_vgl_colors );
	Vita_DrawGLPoly( GL_TRIANGLE_STRIP, numverts, GL_TRUE );
...
#ifdef __vita__
		*(colp++) = color[0]; *(colp++) = color[1]; *(colp++) = color[2]; *(colp++) = 1.f;
		*(uvp++) = 1.f; *(uvp++) = vLast;
		*(vertp++) = point[0]; *(vertp++) = point[1]; *(vertp++) = point[2];
		numverts++;
#else
...
#ifdef __vita__
		*(colp++) = 0.f; *(colp++) = 0.f; *(colp++) = 0.f; *(colp++) = 1.f;
		*(uvp++) = 1.f; *(uvp++) = vLast;
		*(vertp++) = point[0]; *(vertp++) = point[1]; *(vertp++) = point[2];
		numverts++;
#else
	...
	float amplitude, float freq, float *color )
{
	...
	while( pHead )
	{
		...
		*(vertp++) = last1[0]; *(vertp++) = last1[1]; *(vertp++) = last1[2];
		numverts++;
			...
			*(colp++) = saved_nColor[0]; *(colp++) = saved_nColor[1]; *(colp++) = saved_nColor[2]; *(colp++) = 1.f;
			*(uvp++) = 1.f; *(uvp++) = 1.f;
			*(vertp++) = saved_last2[0]; *(vertp++) = saved_last2[1]; *(vertp++) = saved_last2[2];
			numverts++;
	...
	Vita_TexCoordPointer( 2, GL_FLOAT, 0, numverts, gl_vgl_texcoords );
	Vita_ColorPointer( 4, GL_FLOAT, 0, numverts, gl_vgl_colors );
	Vita_DrawGLPoly( GL_TRIANGLES, numverts, GL_TRUE );
	pglDisableClientState( GL_VERTEX_ARRAY );
	pglDisableClientState( GL_TEXTURE_COORD_ARRAY );
	pglDisableClientState( GL_COLOR_ARRAY );
#else
