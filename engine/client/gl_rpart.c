...
#ifdef __vita__
	vec3_t verts[4];
	float uvs[] = { 1, 0,  0, 0,  0, fOffset,  1, fOffset };

	VectorMA( vecStart, -width, cross, verts[0] );
	VectorMA( vecStart, width, cross, verts[1] );
	VectorMA( vecEnd, width, cross, verts[2] );
	VectorMA( vecEnd, -width, cross, verts[3] );
...
#ifdef __vita__
	float verts[] = {
		p->org[0] - right[0] + up[0], p->org[1] - right[1] + up[1], p->org[2] - right[2] + up[2],
		p->org[0] + right[0] + up[0], p->org[1] + right[1] + up[1], p->org[2] + right[2] + up[2],
		p->org[0] + right[0] - up[0], p->org[1] + right[1] - up[1], p->org[2] + right[2] - up[2],
		p->org[0] - right[0] - up[0], p->org[1] - right[1] - up[1], p->org[2] - right[2] - up[2]
	};
...
void CL_DrawTracer( vec3_t start, vec3_t delta, float width, rgb_t color, int alpha, float startV, float endV )
{
	...
	float uvs[] = { 0, endV,  1, endV,  1, startV,  0, startV };
