...
void UI_LoadBackgroundImage( void )
{
	...
	for( int y = 0; y < BACKGROUND_ROWS; y++ )
	{
		for( int x = 0; x < BACKGROUND_COLUMNS; x++ )
		{
			...
			if (g_ui_engfuncs.pfnFileExists( filename, TRUE ))
	...
	{
		if( g_ui_engfuncs.pfnFileExists( "gfx/shell/splash.bmp", TRUE ))
		{
			// if we doesn't have logo.avi in gamedir we don't want to draw it
			if( !g_ui_engfuncs.pfnFileExists( "media/logo.avi", TRUE ))
...
void UI_RefreshServerList( void )
{
	...
	uiStatic.serversRefreshTime = ui_gpGlobals->time;
...
bool UI_StartBackGroundMap( void )
{
	...
	if( !uiStatic.bgmapcount || CL_IsActive() || ui_gpGlobals->demoplayback )
		return FALSE;
...
void UI_PushMenu( menuFramework_s *menu )
{
	...
	uiStatic.enterSound = ui_gpGlobals->time + 0.15;	// make some delay
...
void UI_UpdateMenu( float flTime )
{
	...
	if( CVAR_GET_FLOAT( "cl_background" ) && !g_ui_engfuncs.pfnClientInGame())
	...
	if( uiStatic.enterSound > 0.0f && uiStatic.enterSound <= ui_gpGlobals->time )
	{
...
{
	if( !g_ui_engfuncs.pfnFileExists( "scripts/chapterbackgrounds.txt", TRUE ))
		return;
