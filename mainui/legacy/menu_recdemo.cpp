...
static void UI_RecDemo_GetDemoList( void )
{
	...
	if ( CL_IsActive () && !ui_gpGlobals->demorecording && !ui_gpGlobals->demoplayback )
	{
		char maxClients[32];
		sprintf( maxClients, "%i", ui_gpGlobals->maxClients );

		// create new entry for current save game
		strncpy( uiRecDemo.demoName[i], "new", CS_SIZE );
		StringConcat( uiRecDemo.demoDescription[i], ui_gpGlobals->maptitle, TITLE_LENGTH );
	...
	if( strlen( uiRecDemo.demoName[0] ) == 0 || !CL_IsActive () || ui_gpGlobals->demoplayback )
		uiRecDemo.record.generic.flags |= QMF_GRAYED;
	else uiRecDemo.record.generic.flags &= ~QMF_GRAYED;

	if( strlen( uiRecDemo.delName[0] ) == 0 || !stricmp( ui_gpGlobals->demoname, uiRecDemo.delName[uiRecDemo.demosList.curItem] ))
...
static void UI_RecDemo_Callback( void *self, int event )
{
	...
	if( event == QM_CHANGED )
	{
		...
		if( strlen( uiRecDemo.demoName[uiRecDemo.demosList.curItem] ) == 0 || !CL_IsActive() || ui_gpGlobals->demoplayback )
			uiRecDemo.record.generic.flags |= QMF_GRAYED;
		else uiRecDemo.record.generic.flags &= ~QMF_GRAYED;

		if( strlen( uiRecDemo.delName[uiRecDemo.demosList.curItem] ) == 0 || !stricmp( ui_gpGlobals->demoname, uiRecDemo.delName[uiRecDemo.demosList.curItem] ))
			uiRecDemo.remove.generic.flags |= QMF_GRAYED;
		else uiRecDemo.remove.generic.flags &= ~QMF_GRAYED;
		return;
	}
