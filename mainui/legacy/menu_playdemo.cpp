...
static void UI_PlayDemo_GetDemoList( void )
{
	...
	if( strlen( uiPlayDemo.delName[0] ) == 0 || !stricmp( ui_gpGlobals->demoname, uiPlayDemo.delName[uiPlayDemo.demosList.curItem] ))
...
static void UI_PlayDemo_Callback( void *self, int event )
{
	...
	if( event == QM_CHANGED )
	{
		...
		if( strlen( uiPlayDemo.delName[uiPlayDemo.demosList.curItem] ) == 0 || !stricmp( ui_gpGlobals->demoname, uiPlayDemo.delName[uiPlayDemo.demosList.curItem] ))
			uiPlayDemo.remove.generic.flags |= QMF_GRAYED;
		else uiPlayDemo.remove.generic.flags &= ~QMF_GRAYED;
		return;
	}
	...
	case ID_CANCEL:
		...
		break;
	case ID_PLAY:
		if( ui_gpGlobals->demoplayback || ui_gpGlobals->demorecording )
		{
