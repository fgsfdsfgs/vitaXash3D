...
static void UI_Credits_DrawFunc( void )
{
	...
	y = ScreenHeight - (((ui_gpGlobals->time * 1000) - uiCredits.startTime ) / speed );
	...
	for ( i = 0; i < uiCredits.numLines && uiCredits.credits[i]; i++, y += h )
	{
		...
		{
			if( !uiCredits.fadeTime ) uiCredits.fadeTime = (ui_gpGlobals->time * 1000);
...
static void UI_Credits_Init( void )
{
	...
	// run credits
	uiCredits.startTime = (ui_gpGlobals->time * 1000) + 500; // make half-seconds delay
